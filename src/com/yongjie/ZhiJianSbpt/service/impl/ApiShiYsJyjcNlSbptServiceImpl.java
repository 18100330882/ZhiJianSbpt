...
@Service(ApiShiYsJyjcNlSbptService.SERVICE_NAME)
public class ApiShiYsJyjcNlSbptServiceImpl implements ApiShiYsJyjcNlSbptService {

    @Resource(name = ApiShiYsJyjcNlSbptDao.SERVICE_NAME)
    private ApiShiYsJyjcNlSbptDao dao;

    @Override
    @Transactional(isolation = Isolation.DEFAULT, propagation = Propagation.REQUIRED, readOnly = false)
    public int saveBatch(List<ApiShiYsJyjcNlSbpt> list, int size) throws Exception {
        // 保存 能力、仪表数据 先删除 再 保存（同一个流水号-同一个食品-同一个场地）
        if (list != null && list.size() > 0) {
            ApiShiYsJyjcNlSbpt entity = list.get(0);
    ...
    @Override
    @Transactional(isolation = Isolation.DEFAULT, propagation = Propagation.REQUIRED, readOnly = false)
    public void updateRow(ApiShiYsJyjcNlSbpt entity) {
    ...
    @Override
    @Transactional(isolation = Isolation.DEFAULT, propagation = Propagation.REQUIRED, readOnly = false)
    public void addRow(ApiShiYsJyjcNlSbpt entity) {
        // [检验检测能力] 新增 设置 序号
     ...
     * @return
     */
    private String reSetxhAddRow(ApiShiYsJyjcNlSbpt entity) {
        String result = "";
        HashMap<String, Object> reqMap = new HashMap<>();
        reqMap.put("serialNumber", entity.getSerialNumber());
        reqMap.put("flag", entity.getFlag());
     ...
     */
    @Override
    public List<ApiShiYsJyjcNlSbpt> reSetXh(HashMap<String, Object> reqMap) throws Exception {
        Map<String, Object> detailMap = dao.getDetailList(reqMap);
        List<ApiShiYsJyjcNlSbpt> updateList = new ArrayList<>();
        if (detailMap != null) {
                ...
                for (int i = 0; i < flList.size(); i++) {
                    ...
                    for (int j = 0; j < detailList.size(); j++) {
                        ...
                        if (dbEjfl.equals(key)) {
                            ...
                            ApiShiYsJyjcNlSbpt entity = JSON.parseObject(JSON.toJSONString(dbMap), ApiShiYsJyjcNlSbpt.class);
    ...
    @Override
    @Transactional(isolation = Isolation.DEFAULT, propagation = Propagation.REQUIRED, readOnly = false)
    public void updateList(List<ApiShiYsJyjcNlSbpt> entityList) {
        if (entityList != null && entityList.size() > 0) {
            for (ApiShiYsJyjcNlSbpt entity : entityList) {
                dao.update(entity);
            }
        }
    }
    ...
    public Map<String, Object> queryList(HashMap<String, Object> reqMap) {
        return dao.queryList(reqMap);
    }

    @Override
    public ArrayList<ApiShiYsJyjcNlSbpt> queryListByTj(Map map) throws Exception {
        return dao.queryListByTj(map);
    }


    @Override
    public List queryListBySelect(Map map) {
        return dao.queryListBySelect(map);
    }

    @Override
    public List queryYjfl(Map map) {
        return dao.queryYjfl(map);
    }
     ...
     */
    @Override
    public List<ApiShiYsJyjcNlSbpt> reSetXhYqsb(HashMap<String, Object> reqMap) throws Exception {
        Map<String, Object> detailMap = dao.getDetailList(reqMap);
        List<ApiShiYsJyjcNlSbpt> updateList = new ArrayList<>();
        if (detailMap != null) {
            List<Map<String, Object>> detailList = (List<Map<String, Object>>) detailMap.get(R.DATA_NAME);
     ...
     * @return
     */
    private String reSetxhAddRowYqsb(ApiShiYsJyjcNlSbpt entity) {
        String result = "";
        HashMap<String, Object> reqMap = new HashMap<>();
        reqMap.put("serialNumber", entity.getSerialNumber());
        reqMap.put("flag", entity.getFlag());
