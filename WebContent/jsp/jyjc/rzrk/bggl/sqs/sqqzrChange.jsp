...
<form id="form" target="id_iframe" style="padding-top:5px;margin:0 auto; width:95%; border:1px;">
    ...
    <fieldset style="border: solid 1px #aaa; padding: 3px; width:95%;">
        <div style="width:95%;">
            <table width="100%">
                <tr>
                    <td align="right" width="30%">检验检测机构名称：<span style="color:red;">*</span></td>
                    <td>
                ...
                <tr>
                    ...
                    <td align="left" colspan="4">本机构自我承诺，变更后的授权签字人符合《检验检测机构资质认定评审
                        检验检测机构通用要求》(RB/T214-2017)的要求，并对真实性负责。
                    </td>
                </tr>
                <tr>
        ...
        $.ajax({
            ...
            success: function (text) {
                ...
                if (map.code == 0) {
                    ...
                    nui.get("address").setValue(data.registerAdress);
                    nui.get("address").setValue(data.registerAdress);
                }
            }
        });
    ...
    function add() {
        nui.open({
            ...
            onload: function () {
                ...
                //新窗口传递json数据
                var data = {changeType: changeType, serialNumber: serialNumber, zsbh: zsbh, action: 'add'};
                //调用新窗口的SetData方法。
    ...
    function edit() {
        ...
        nui.open({
            ...
            onload: function () {
                ...
                var iframe = this.getIFrameEl();
                //新窗口传递json数据
                var data = {row: rows[0], zsbh: zsbh, changeType: changeType, serialNumber: serialNumber};
                //调用新窗口的SetData方法。
                iframe.contentWindow.SetData(data);
            },
            ondestroy: function (action) {
