        ...
        <table style="width:100%;">
            <tr>
                <td style="width: 100%;">
                    ...
                    <a id="moreInfo" class="nui-button" iconCls="icon-download" onclick="moreInfo()">相关信息下载</a>
...
<div id="dataGrid" class="nui-datagrid" style="width:100%;height:95%;" allowResize="false" idField="ID" multiSelect="true" sizeList="[50,100,500]" showPager="false" pageSize="50"
...
<script type="text/javascript">
    ...
    grid.load("<%=basePath%>apiShiYsJyjcNl_getNlFileList.action?nlorSb=0&serialNumber=" + serialNumber);
    grid.reload();
    ...
    function scword() {
        ...
        $.ajax({
            url:"<%=basePath%>apiShiYsJyjcNl_returnNlWord.action",
            type: 'post',
            cache: false,
            data:{psbgId:row.ID},
        ...
        //window.open("<%=basePath%>jsp/jyjc/rzrk/zzrd/sqs/nlsaveword.jsp?psbgId=" + row.ID)
    }
    ...
    function  uploadInfo() {
        nui.open({
            ...
            onload: function () {
                ...
                //新窗口传递json数据
                var data = {serialNumber: serialNumber, flowId: "4", type: "0"};
                //调用新窗口的SetData方法。
    ...
    function ondrawcellEvent(e) {
        ...
        if ("LOOKFILE" == field) {
            e.cellHtml =
                '<a class="nui-button mini-button-c1" onclick="previewEvent(\'' + param + '\')" style="cursor: pointer" href="javascript:void(0)">预览</a>';
        }
        /*if ("EDITFILE" == field) {
            e.cellHtml =
        ...
        if ("DELETEFILE" == field) {
            e.cellHtml =
                '<a class="nui-button mini-button-c1" id="deleteId" onclick="deleteEvent(\'' + record.ID + '\')" style="cursor: pointer" href="javascript:void(0)">删除</a>';
            if (action == "detail") {
                e.cellHtml =
                    '<a class="nui-button mini-button-c1" id="deleteRow" style="color: #7D808D" href="javascript:void(0)">删除</a>';
            }
        }
    }
    ...
    function deleteEvent(id) {
        nui.confirm("如果删除Excel文件，需要重新上传Excel文件!", "提醒", function (action) {
            if ("ok" == action) {
                $.ajax({
                    url: "<%=basePath%>apiShiYsJyjcNl_deleteNlsbptEvent.action",
                    type: 'post',
                    cache: 'false',
                    async: false,
                    data: {id: id},
