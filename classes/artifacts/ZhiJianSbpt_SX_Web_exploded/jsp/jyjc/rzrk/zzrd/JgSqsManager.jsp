...
<div id="listGrid" class="nui-datagrid" style="width:100%;height:95%;" allowResize="false" idField="ID" multiSelect="true" sizeList="[50,100,500]" showPager="true" pageSize="50"
...
<script type="text/javascript">
    ...
    grid.load("<%=basePath%>bizApiShiysjyjc_getList.action?flowId=" + flowId);
    grid.reload();
                ...
                if (action == "ok") {
                    ...
                    $.ajax({
                        ...
                        success: function (resultData) {
                            ...
                            nui.open({
                                url: "<%=basePath%>jsp/jyjc/rzrk/zzrd/sqs/bizSqlx.jsp?serialNumber=" + serialNumber,
                                title: "选择申请类型", width: "40%", height: "40%",
                                ondestroy: function (sqlx) {
                                    if (sqlx != "close") {
                                        nui.open({
                                            url: "<%=basePath%>jsp/jyjc/rzrk/zzrd/stepInfo.jsp?sqlx=" + encodeURI(encodeURI(sqlx)),
    ...
    function edit() {
        ...
        var applicationType;
        nui.open({
            url: "<%=basePath%>jsp/jyjc/rzrk/zzrd/sqs/bizSqlx.jsp?serialNumber=" + row.SERIALNUMBER,
            title: "选择申请类型", width: "40%", height: "40%",
            ondestroy: function (sqlx) {
                if (sqlx != 'close') {
                    nui.open({
                        ...
                        onload: function () {
                            var iframe = this.getIFrameEl();
                            var data = {action: "edit", serialNumber: row.SERIALNUMBER, flowId: row.FLOWID};
                            iframe.contentWindow.SetData(data);
                        },
                        ondestroy: function (action) {
                            grid.reload();
    ...
    function addfile() {
        ...
        if (row) {
            nui.open({
                url: "<%=basePath%>jsp/jyjc/rzrk/zzrd/fuJian/apiFuJian.jsp?serialNumber=" + row.SERIALNUMBER + "&flowId=" + row.FLOWID + "&sqsType=检验检测",
                title: "附件管理",
                width: '70%',
                height: 800,
                ondestroy: function (action) {
    ...
    function shangBao() {
        ...
        nui.confirm("申请材料上报后不能做修改，如需修改请联系审批部门退回！请确认提交前内容正确！", "温馨提示", function (action) {
            if (action == "ok") {
                for (var i = 0; i < rows.length; i++) {
                    //提交时只修改当前数据的Flag
                    var row = rows[i];
