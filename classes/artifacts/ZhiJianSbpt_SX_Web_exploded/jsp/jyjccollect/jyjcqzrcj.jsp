...
<div style="width:100%;">
    <div class="nui-toolbar" style="border-bottom:0;padding:0px;">
        <table style="width:100%;">
            <tr>
                <td style="width: 100%;">
                    <a id="downLoadTemp" class="nui-button" iconCls="icon-download" onclick="downLoadTemp()">下载检验检测签字人信息模板</a>
                    <a id="uploadInfo" class="nui-button" iconCls="icon-upload" onclick="uploadInfo()">上传检验检测签字人信息</a>
                </td>
            </tr>
        </table>
    </div>
...
<script type="text/javascript">
    ...
    function downLoadTemp() {
        var fileName = "qzrTemp";
        location = "<%=basePath%>jsp/common/downjyjccjMuban.jsp?status=" + 2;
    ...
    function uploadInfo() {
        nui.open({
            url: "<%=basePath%>jsp/jyjccollect/fileupload.jsp?status=" + 2 + "&zsbh=" + zsbh,
            title: "添加附件",
            width: 800,
            height: 300,
    ...
    function cksj(id) {
        nui.open({
            url: "<%=basePath%>jsp/jyjccollect/qzrcksj.jsp",
    ...
    function del(id) {
        nui.confirm("如果删除Excel文件，需要重新上传Excel文件!", "提醒", function (action) {
            if (action == "ok") {
                $.ajax({
                    url: "<%=basePath%>collect_delqzr.action?id=" + id + "&zsbh=" + zsbh,
                    type: "post",
    ...
    $(function () {
        $.ajax({
            ...
            success: function (result) {//成功获得数据时
                ...
                grid.load("<%=basePath%>collect_ckfj.action?zsbh=" + zsbh + "&state=" + 2);
                grid.reload();
